import { SiteFooter } from "@/components/layout/site-footer";
import { SiteHeader } from "@/components/layout/site-header";

export default function StudioPage() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      <SiteHeader />
      <main className="container space-y-6 py-16 md:py-24">
        <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Studio</p>
        <h1 className="max-w-4xl text-balance text-[clamp(2rem,4.8vw,4.8rem)] font-semibold leading-[1.02] tracking-[-0.02em]">
          We design timeless spaces with strategy, precision and emotional clarity.
        </h1>
        <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground md:text-base">
          Studio page is now enabled as a primary route and will receive full editorial composition in the next step.
        </p>
      </main>
      <SiteFooter />
    </div>
  );
}
